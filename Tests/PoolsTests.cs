...
public class PoolsTests
{
    ...
    [Fact]
    public async Task TestPools_ListPoolsPoolList()
    {

        var testHttpClient = CommonHelpers.CreateTestHTTPClient("listPools-pool_list");
    ...
    [Fact]
    public async Task TestPools_GetPool()
    {

        var testHttpClient = CommonHelpers.CreateTestHTTPClient("getPool");
        var sdk = new NewlineSDK(
            serverUrl: Environment.GetEnvironmentVariable("TEST_SERVER_URL") ?? "http://localhost:18080",
            security: new Security() {
                ProgramUid = "NEWLINE_PROGRAM_UID",
